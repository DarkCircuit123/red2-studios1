 ...
 * Usage:
 * ```typescript
 * import { runImageHealthScan } from '@/lib/image-health-scanner';
 * const report = await runImageHealthScan();
 * console.log(report);
 * ```
 */
...
const WIX_IMAGE_URL_PATTERN = /^https:\/\/static\.wixstatic\.com\/media\//;
...
interface ImageHealthScanReport {
  timestamp: string;
...
function validateImageUrl(url: string | null | undefined): {
  ...
  // Check for Wix URL format
  if (!WIX_IMAGE_URL_PATTERN.test(url)) {
    issues.push({
      severity: 'ERROR',
      code: 'INVALID_URL_FORMAT',
      message: `Image URL does not match Wix format: ${url.substring(0, 50)}...`,
      recommendation: 'Use Wix Media Manager to upload images. URLs should start with https://static.wixstatic.com/media/',
    });
    return { isValid: false, issues };
  }
 ...
 */
export async function runImageHealthScan(): Promise<ImageHealthScanReport> {
  ...
  for (const mapping of IMAGE_FIELD_MAPPINGS) {
    console.log(`  Scanning ${mapping.collectionName}...`);
    const reports = await scanCollection(mapping);
 ...
 */
export async function getImageHealthStatus(): Promise<{
  status: 'PASS' | 'FAIL' | 'WARNING';
  passCount: number;
  failCount: number;
  warningCount: number;
