...
export default function AdminDashboard() {
  ...
  useEffect(() => { console.log('[AdminDashboard] activeTab:', activeTab); }, [activeTab]);
  ...
  return <div className="min-h-screen bg-admin-bg">
    <div className="border-b border-admin-line bg-admin-bg"><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6"><div className="flex items-center gap-3"><h1 className="font-heading text-[22px] uppercase tracking-[0.18em] text-admin-text">RED<span className="text-oxblood relative -top-1">²</span></h1><div className="w-px h-6 bg-admin-line" /><p className="text-[11px] uppercase tracking-[0.2em] text-admin-faint">CONTROL ROOM</p></div></div></div>
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8"><div className="bg-admin-surface border border-admin-line"><Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
      <div className="border-b border-admin-line bg-admin-surface overflow-x-auto"><TabsList className="w-full justify-start bg-transparent p-0 h-auto rounded-none">{tabs.map((tab) => { const Icon = tab.icon; return <TabsTrigger key={tab.id} value={tab.id} disabled={!tab.enabled} className={`flex items-center gap-2 px-4 py-3 border-b-2 rounded-none font-heading text-[11px] uppercase tracking-[0.12em] ${!tab.enabled ? 'opacity-50 cursor-not-allowed' : ''}`}><Icon className="w-4 h-4" /><span>{tab.label}</span>{!tab.enabled && <span className="text-[11px] text-admin-faint ml-1">(Coming soon)</span>}</TabsTrigger>; })}</TabsList></div>
      ...
      <TabsContent value="photo-library" className="m-0 p-6"><ProfessionalPhotoLibrary /></TabsContent>
      ...
      {['about', 'services', 'booking', 'contact', 'settings'].map((tabId) => <TabsContent key={tabId} value={tabId} className="m-0 p-6"><div className="text-center py-12"><p className="text-admin-dim text-[13px]">This tab is coming soon.</p></div></TabsContent>)}
    </Tabs></div></div>
  </div>;
}
