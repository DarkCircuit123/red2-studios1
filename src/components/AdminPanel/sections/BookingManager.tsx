...
import { Plus, Trash2, Edit2, X, Check, AlertCircle, Eye, Calendar } from 'lucide-react';
import { motion } from 'framer-motion';
import { BaseCrudService } from '@/integrations';
import { Bookings, BookingAvailability } from '@/entities';
...
export default function BookingManager() {
  ...
  const [editingForm, setEditingForm] = useState<BookingForm>({
    clientName: '',
    clientEmail: '',
    clientPhone: '',
    sessionType: '',
  ...
  const [editingAvailabilityForm, setEditingAvailabilityForm] = useState<AvailabilityForm>({
    bookingDate: '',
    startTime: '',
    endTime: '',
    sessionType: '',
  ...
  const loadData = async () => {
    try {
      ...
      if (activeTab === 'bookings') {
        ...
        console.log('[BookingManager] Loaded', result.items?.length || 0, 'availability slots');
      }
    } catch (error) {
      console.error('[BookingManager] Error loading data:', error);
      addStatusMessage('error', `Failed to load ${activeTab}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleEditAvailability = (slot: BookingAvailability) => {
    setEditingId(slot._id);
    setEditingAvailabilityForm({
      bookingDate: slot.bookingDate ? new Date(slot.bookingDate).toISOString().split('T')[0] : '',
  ...
  const handleSaveAvailability = async () => {
    if (!editingAvailabilityForm.bookingDate || !editingAvailabilityForm.startTime || !editingAvailabilityForm.endTime) {
      addStatusMessage('error', 'Date and times are required');
      return;
    }
    ...
    try {
      ...
      if (editingId === 'NEW') {
        await BaseCrudService.create('bookingavailability', {
          _id: crypto.randomUUID(),
          bookingDate: editingAvailabilityForm.bookingDate,
          startTime: editingAvailabilityForm.startTime,
          endTime: editingAvailabilityForm.endTime,
          sessionType: editingAvailabilityForm.sessionType,
          isAvailable: editingAvailabilityForm.isAvailable,
        });
        addStatusMessage('success', 'Availability slot created successfully');
      } else if (editingId) {
        ...
        addStatusMessage('success', 'Availability slot updated successfully');
      }

      await loadData();
      handleCancel();
    } catch (error) {
      console.error('[BookingManager] Error saving availability:', error);
      addStatusMessage('error', 'Failed to save availability slot');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDeleteAvailability = async (id: string) => {
    ...
    try {
      ...
      addStatusMessage('success', 'Availability slot deleted successfully');
      await loadData();
    } catch (error) {
      console.error('[BookingManager] Error deleting availability:', error);
      addStatusMessage('error', 'Failed to delete availability slot');
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2 border-b border-admin-line">
        <button
          ...
          className={`px-4 py-3 font-heading text-[11px] uppercase tracking-[0.12em] border-b-2 transition-colors ${
            ...
            <>
              ...
              {editingId !== null && (
                ...
                >
                  <h3 className="text-lg font-semibold text-admin-text">
                    {editingId === 'NEW' ? 'Add New Availability Slot' : 'Edit Availability Slot'}
                  </h3>
              ...
              <div className="space-y-3">
                {availability.length === 0 ? (
                  <p className="text-center text-admin-dim py-8">No availability slots yet.</p>
                ) : (
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-2">
                            <Calendar className="w-4 h-4 text-admin-dim" />
                          ...
                          {slot.sessionType && (
                            <span className="text-xs px-2 py-1 bg-blue-900/30 text-blue-400 rounded inline-block mt-2">
                              {slot.sessionType}
                            </span>
                          )}
                          <span className={`text-xs px-2 py-1 rounded inline-block ml-2 ${
                            slot.isAvailable ? 'bg-green-900/30 text-green-400' : 'bg-red-900/30 text-red-400'
                          }`}>
