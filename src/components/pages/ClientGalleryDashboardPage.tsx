...
import { LogOut, Image as ImageIcon, AlertCircle } from 'lucide-react';
...
export default function ClientGalleryDashboardPage() {
  ...
  useEffect(() => {
    // Check if user is logged in with account
    if (!clientSession?.isAccountLogin) {
  ...
  const loadGalleries = async () => {
    try {
      ...
      // Filter galleries for this client
          ...
          >
            <div className="flex items-center justify-between mb-8">
              ...
              <button
                ...
                className="flex items-center gap-2 px-6 py-3 bg-white/10 hover:bg-white/20 border border-white/20 rounded-lg text-white font-heading font-bold text-sm tracking-widest uppercase transition-all duration-300"
              >
                ...
                >
                  <div className="relative overflow-hidden rounded-lg border border-white/10 hover:border-white/30 transition-all duration-300 h-64 bg-white/5 flex items-center justify-center">
                    ...
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                      <button className="px-6 py-2 bg-white text-black font-heading font-bold text-sm tracking-widest uppercase rounded-lg hover:bg-white/90 transition-all duration-300">
                        View Gallery
                      </button>
                    </div>
                  ...
                  <div className="mt-4">
                    ...
                    <p className="text-sm text-white/60">
                      Access Code: <span className="font-mono text-white/80">{gallery.galleryAccessCode}</span>
                    </p>
                  </div>
                </motion.div>
              ))}
