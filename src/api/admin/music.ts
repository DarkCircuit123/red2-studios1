...
export const GET: APIRoute = async (context) => {
  ...
  try {
    const result = await BaseCrudService.getAll<MusicSettings>('musicsettings', {}, { limit: MAX_TRACKS });
    return json({ success: true, items: result.items || [] });
  } catch (error) {
    console.error('[ADMIN_MUSIC] Read failed:', error instanceof Error ? error.message : String(error));
...
export const POST: APIRoute = async (context) => {
  ...
  try {
    ...
    if (body.action === 'create') {
      ...
      const current = await BaseCrudService.getAll<MusicSettings>('musicsettings', {}, { limit: MAX_TRACKS });
    ...
    if (body.action === 'update') {
      ...
      const current = await BaseCrudService.get<MusicSettings>('musicsettings', body.trackId);
      ...
      for (const key of ['musicTitle', 'artist', 'album', 'genre', 'duration', 'musicUrl'] as const) {
        ...
        if (typeof value === 'string') allowed[key] = value.slice(0, key === 'musicUrl' ? 4096 : 200) as never;
      }
      ...
      await BaseCrudService.update<MusicSettings>('musicsettings', { _id: body.trackId, ...allowed });
      return json({ success: true, track: { ...current, ...allowed, _id: body.trackId } });
    }
    ...
    if (body.action === 'set-active') {
      ...
      for (const track of result.items || []) {
        ...
        if (track.isEnabled !== active || track.isDefaultHomepageTrack !== active) {
          await BaseCrudService.update<MusicSettings>('musicsettings', { _id: track._id, isEnabled: active, isDefaultHomepageTrack: active });
        }
      }
    ...
    console.error('[ADMIN_MUSIC] Write failed:', error instanceof Error ? error.message : String(error));
