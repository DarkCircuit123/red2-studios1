...
interface UploadGalleryResponse { success: true; mediaUrl: string; fileId?: string; }
...
function isSafeMediaUrl(value: unknown): value is string {
  if (typeof value !== 'string' || !value) return false;
  if (value.startsWith('wix:image://') || value.startsWith('wix:audio://')) return true;
  try {
    const url = new URL(value);
    return url.protocol === 'https:' && !url.username && !url.password;
  } catch { return false; }
...
export const POST: APIRoute = async (context) => {
  ...
  try {
    ...
    if (entry.size <= 0 || entry.size > MAX_SIZE_BYTES) {
      return jsonResponse({ success: false, error: entry.size > MAX_SIZE_BYTES ? 'Image is too large. Maximum file size is 10MB.' : 'Image file is empty.' }, 400);
    }

    let mimeType: string;
    try { mimeType = detectMimeType(entry); }
    catch (error) { return jsonResponse({ success: false, error: error instanceof Error ? error.message : 'Unsupported image type.' }, 400); }
    ...
    if (!isSafeMediaUrl(mediaUrl)) {
      console.error(`[UPLOAD_GALLERY] ${requestId} Wix returned an invalid media result`);
      return jsonResponse({ success: false, error: 'Wix returned an invalid media result.' }, 502);
    }

    console.log(`[UPLOAD_GALLERY] ${requestId} completed`, { mimeType, sizeBytes: entry.size, fileId: typeof fileId === 'string' ? fileId : undefined, durationMs: Date.now() - startTime });
    return jsonResponse({ success: true, mediaUrl, fileId: typeof fileId === 'string' ? fileId : undefined }, 200);
  } catch (error) {
    console.error(`[UPLOAD_GALLERY] ${requestId} failed`, { error: error instanceof Error ? error.message : String(error), durationMs: Date.now() - startTime });
