...
function isSafeMediaUrl(value: unknown): value is string {
  ...
  if (/^wix:(?:image|audio):\/\//.test(value)) return true;
...
async function probeUrl(url: string): Promise<Response> {
  ...
  try {
    ...
    if (response.status === 405 || response.status === 501) {
      ...
      response = await fetch(url, { method: 'GET', redirect: 'manual', headers: { Range: 'bytes=0-0' }, signal: controller.signal });
    }
...
export const POST: APIRoute = async (context) => {
  ...
  try {
    ...
    if (typeof rawUrl !== 'string' || rawUrl.trim().length === 0 || rawUrl.trim().length > 2048) return jsonResponse({ error: 'Paste a valid file link first.' }, 400);
    let parsed: URL;
    try { parsed = new URL(rawUrl.trim()); } catch { return jsonResponse({ error: "That's not a valid web address." }, 400); }
    ...
    try { probeResponse = await probeUrl(parsed.toString()); }
    catch (error) {
      console.warn(`[IMPORT_FROM_URL] ${requestId} URL probe failed`, { reason: error instanceof Error ? error.message : String(error) });
    ...
    } catch { fileName = `${kind}-import`; }

    let importResult;
    try {
      const elevatedImportFile = auth.elevate(files.importFile);
      importResult = await elevatedImportFile(parsed.toString(), { mimeType: detectedType, displayName: fileName, mediaType: kind === 'music' ? 'AUDIO' : 'IMAGE' });
    } catch (importError) {
      console.error(`[IMPORT_FROM_URL] ${requestId} Wix import failed`, { reason: importError instanceof Error ? importError.message : String(importError) });
    ...
    if (!isSafeMediaUrl(mediaUrl) || typeof mediaId !== 'string' || !mediaId) {
    ...
    console.log(`[IMPORT_FROM_URL] ${requestId} completed`, { kind, detectedType, detectedSizeBytes: detectedSize, mediaId, durationMs: Date.now() - startTime });
    return jsonResponse({ success: true, mediaUrl, mediaId, fileName, detectedType, detectedSizeBytes: detectedSize, pending: importResult.file?.operationStatus === 'PENDING', message: 'Link verified and imported into Media Manager.' }, 200);
  } catch (error) {
    console.error(`[IMPORT_FROM_URL] ${requestId} failed`, { reason: error instanceof Error ? error.message : String(error), durationMs: Date.now() - startTime });
